...
_LEAD_NAMES = ['I', 'II', 'III', 'AVR', 'AVL', 'AVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']##PTB-XL
...
def get_parser():
    ...
    parser.add_argument('-i',
                        ...
                        type=str,
                        # required=True,
                        default="/mnt/DataSets/phan635/ECG_datasets/ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/records500/",
                        help="Path to the WFDB ECG database directory.")
    parser.add_argument('-o',
                        '--output_dir',
                        type=str,
                        # required=True,
                        default="/mnt/DataSets/phan635/ECG_datasets/processed_data/",
...
def moving_window_crop(x: np.ndarray, crop_length: int, crop_stride: int) -> np.ndarray:
    ...
    """
    if crop_length > x.shape[1]:##PTB-XL: 5000
    ...
    # print('start_idx',start_idx)##PTB-XL: [0]
...
def run(args):
    ...
    for record_rel_path in tqdm(record_rel_paths):
        # print(record_rel_path)##PTB-XL: 0000/00001_hr
        record_rel_dir, record_name = os.path.split(record_rel_path)
        # print(record_rel_dir, record_name)##PTB-XL: 0000 00001_hr
        ...
        # print('record_info["sig_name"]',record_info["sig_name"])##PTB-XL: ['I', 'II', 'III', 'AVR', 'AVL', 'AVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']
        
        ##_LEAD_NAMES = ["I", "II", "III", "aVR", "aVL", "aVF", "V1", "V2", "V3", "V4", "V5", "V6"]
        lead_idx = np.array([record_info["sig_name"].index(lead_name) for lead_name in _LEAD_NAMES])
        signal = signal[:, lead_idx]
        # print(signal.shape)##PTB-XL: (5000, 12)
        fs = record_info["fs"]
        # print(f"fs: {fs}Hz")##PTB-XL: 500Hz
        ...
        # print(len(cropped_signals))##PTB-XL: 1
